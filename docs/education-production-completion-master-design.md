...
当前收口进度（2026-09-19）：备课资料、教案附件、课件、教学材料和教研资料已统一采用“文件草稿上传 -> 业务权限校验 -> 服务端正式绑定”模式；已补教学任务和教研组维度的附件读取策略。课件、材料和题目的提交状态切换、审核记录与工作流启动已合并到同一事务，作者端手工批准/发布入口已关闭，流程通过后自动发布不可变版本，驳回状态保留为 `REJECTED`。备课与教研已使用账号到教师档案的稳定绑定进行本人判断，备课主持人、教研组长和活动主持人的专属操作已在服务端校验。教研活动邀请、取消及成果审核结果已写入平台 Outbox；成果提交使用客户端稳定幂等键，网络重试不会重复启动工作流。作业与考试均已建立“确认业务事实后产生 `WrongAnswerConfirmed`”的链路：作业在成绩发布时产生，考试新增 `DRAFT -> CONFIRMED -> PUBLISHED` 独立成绩状态机，确认后冻结逐题得分、发布后产生错题。事件写入独立 `edu_domain_outbox`，与业务事务原子提交，消费者使用数据库跳锁领取、稳定事件键去重、指数退避和死信重试/忽略；流程消息死信页面可切换查看教育领域死信。考试场次、作业和提交均增加乐观锁，阻止并发确认、评分和发布静默覆盖；重复发布作业成绩会准确返回本次新增发布数量。旧作业快照缺少题目满分时仅确认零分题，避免误判部分得分。知识点维护已分离审核状态与启停开关，停用节点仍可在维护树中重新启用。门户教学中心已移除失效的通用 Map CRUD，改为九个强类型领域工作台入口。旧 `edu_teaching_center_resource` 已关闭全部写入和审核入口，仅保留鉴权后的历史只读查询；历史数据向新领域表的映射仍需按实际数据库盘点后专项迁移，禁止无依据自动转换。
...
当前收口进度（2026-09-18）：会议材料、受邀人签到、纪要发布、决议、行动项及负责人状态反馈已经落地，文件权限和消息通知已接入公共能力。周期会议、外部日历同步继续保留为后续增强项，不阻断当前 MVP 会议闭环。
