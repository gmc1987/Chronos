...
当前收口进度（2026-09-19）：学生档案普通编辑已禁止直接修改学籍状态、年级、专业和行政班。休学、复学、转出、退学、毕业、转班和留级统一通过独立学籍异动记录申请；同一学生只允许一条待审批记录，审批使用行锁和原始档案快照阻止并发覆盖。当天生效的异动在批准事务中同步学生主档与教学班成员，未来日期进入 `APPROVED_PENDING` 并由每日调度器到期生效；操作写入审计日志。异动类型及转出状态均来自数据字典，管理页面提供申请、历史、批准和驳回入口。

教师门户调代课周历已在同一发布课表事实源上落地。周历按日期叠加调课、停课、代课和补课例外；代课教师使用日期例外中的实际教师 ID 获得课程可见性，解决其不属于原教学任务时看不到临时代课的问题。学生和家长仍严格按教学班成员及监护绑定过滤，接口限制单次最多查询 31 天，避免无界展开整学期课表。

班主任工作台已聚合本人所带行政班、学生学籍状态、监护关系覆盖和待关注学籍异动，并提供课表、流程及通知公告入口。接口不信任前端传入的班级 ID，每次依据账号教师绑定和行政班班主任关系反向验证，越权访问其他班级返回 403。当前工程尚无正式家校通知与回执领域，因此工作台不展示虚构的通知阅读率；该指标将在家校领域表及消息回执闭环落地后接入。

教师任职生命周期已将普通档案维护与单位调动、停职、复职、离职分离。异动保留原状态、目标状态、原单位、目标单位、原因和生效日期；未来日期由调度器到期执行，使用行锁、主档快照和乐观版本阻止并发覆盖。停职或离职存在有效任教关系、开课任务或班主任职责时拒绝生效，要求先完成业务交接；生效后同步教师可用状态和教育身份绑定。

家校通知与公告阅读记录已经分离。班主任通知发布时冻结行政班内在籍学生、监护人和有效门户账号快照，并通过平台可靠消息 Outbox 投递站内通知；后续监护关系变化不会篡改历史送达证据。家长只能通过本人收件快照读取通知，同一账号监护多个学生时一次确认会幂等回执全部对应记录。班主任工作台展示实际接收人数和回执人数，截止时间后禁止补交回执。

请假业务在原 Flowable 审批完成台账上增加了独立销假状态机。教师和学生只能查询本人台账并对尚未销假的已批准记录提交一次销假；管理员必须同时具备管理权限和全校数据范围才能查看、通过或驳回。通过后请假台账转为 `CANCELLED`，原流程实例和申请快照保持不变；统计接口直接按日期重叠条件查询数据库，不在内存扫描全表。

### 11.3 教学中心

按 `education-teaching-center-production-redesign.md` 完成教学计划、教案、备课、资源、题库、知识点、教研和错题的逐模块验收。发布版本与修订草稿分离，下游只引用不可变版本。

当前收口进度（2026-09-19）：备课资料、教案附件、课件、教学材料和教研资料已统一采用“文件草稿上传 -> 业务权限校验 -> 服务端正式绑定”模式；已补教学任务和教研组维度的附件读取策略。课件、材料和题目的提交状态切换、审核记录与工作流启动已合并到同一事务，作者端手工批准/发布入口已关闭，流程通过后自动发布不可变版本，驳回状态保留为 `REJECTED`。备课与教研已使用账号到教师档案的稳定绑定进行本人判断，备课主持人、教研组长和活动主持人的专属操作已在服务端校验。教研活动邀请、取消及成果审核结果已写入平台 Outbox；成果提交使用客户端稳定幂等键，网络重试不会重复启动工作流。作业与考试均已建立“确认业务事实后产生 `WrongAnswerConfirmed`”的链路：作业在成绩发布时产生，考试新增 `DRAFT -> CONFIRMED -> PUBLISHED` 独立成绩状态机，确认后冻结逐题得分、发布后产生错题。事件写入独立 `edu_domain_outbox`，与业务事务原子提交，消费者使用数据库跳锁领取、稳定事件键去重、指数退避和死信重试/忽略；流程消息死信页面可切换查看教育领域死信。考试场次、作业和提交均增加乐观锁，阻止并发确认、评分和发布静默覆盖；重复发布作业成绩会准确返回本次新增发布数量。旧作业快照缺少题目满分时仅确认零分题，避免误判部分得分。知识点维护已分离审核状态与启停开关，停用节点仍可在维护树中重新启用。门户教学中心已移除失效的通用 Map CRUD，改为九个强类型领域工作台入口。旧 `edu_teaching_center_resource` 已关闭全部写入和审核入口，仅保留鉴权后的历史只读查询。2026-09-19 对实际 `ChronosEducation` 数据库盘点结果为 0 行，不存在需要转换的历史数据，因此不生成无依据的迁移脚本；旧表继续只读保留用于兼容，所有新增数据必须写入新的强类型领域表。

统一死信中心已进一步拆分“教学错题事件”和“成绩发布事件”两个来源。成绩发布事件提供分页查看、人工重试和忽略操作，并通过乐观锁防止后台投递器与管理员并发覆盖状态；每次调度领取还会生成唯一 `claimToken`，租约过期工作线程的迟到结果不能覆盖新一轮投递状态。三个死信标签统一返回平台 `PageView` 分页契约，不再暴露不稳定的 Spring Data `PageImpl` JSON。
...
教室使用申请已经复用 Flowable 正式审批链路，不在教育模块内重复实现审批状态机。流程发起前按当前账号数据范围校验学期和教室，并检查容量、维修停用、已发布课表、考试安排和已有申请；审批完成后在学期级数据库事务锁内重新校验最终表单快照，确保审批期间新增的排课或考试不会产生双重占用。

审批成功形成 `edu_classroom_reservation` 资源台账，工作流实例号提供幂等约束。回写失败不会伪装成功，而是记录为 `APPLY_FAILED`，管理端可在冲突解除后重试。申请人门户提供本人申请查询和使用日前撤销，撤销会释放后续冲突校验中的资源占用；关键动作均写入平台审计并通过工作流可靠消息通知申请人。
