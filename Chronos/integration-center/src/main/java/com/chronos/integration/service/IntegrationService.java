...
public class IntegrationService {
 ...
 @Transactional public SyncRun startRun(String jobId, String owner) { SyncJob job=jobs.findById(jobId).orElseThrow(); LocalDateTime now=LocalDateTime.now(); if (job.getLeaseUntil()!=null && job.getLeaseUntil().isAfter(now)) throw new IllegalStateException("sync job is leased"); job.setLeaseOwner(owner);job.setLeaseUntil(now.plusMinutes(5)); jobs.save(job); SyncRun run=new SyncRun();run.setJobId(jobId);run.setStartedAt(now);return runs.save(run); }
 public SyncRun execute(String jobId, String owner) { SyncRun run=startRun(jobId,owner); SyncJob job=jobs.findById(jobId).orElseThrow(); Connector c=connectors.findById(job.getConnectorId()).orElseThrow(); int max=Math.max(1,Math.min(10,c.getMaxRetries()==null?3:c.getMaxRetries())); String idempotencyKey=job.getIdempotencyKeyTemplate()==null||job.getIdempotencyKeyTemplate().isBlank()?run.getId():job.getIdempotencyKeyTemplate().replace("{runId}",run.getId()); for(int attempt=1;attempt<=max;attempt++){ run.setAttemptCount(attempt); try { client.get().uri(URI.create(c.getBaseUrl()+job.getRequestPath())).header(HttpHeaders.ACCEPT,MediaType.APPLICATION_JSON_VALUE).header("Idempotency-Key",idempotencyKey).retrieve().toBodilessEntity().timeout(Duration.ofMillis(Math.max(1000,c.getTimeoutMs()))).block(); run.setSuccessCount(1); run.setStatus("SUCCEEDED"); run.setFinishedAt(LocalDateTime.now()); return runs.save(run); } catch(Exception ex) { run.setErrorMessage(safeMessage(ex)); if(attempt<max) try { Thread.sleep(Math.min(30000L,250L << Math.min(attempt,7))); } catch(InterruptedException interrupted){Thread.currentThread().interrupt();break;} } } String message=run.getErrorMessage()==null?"external request failed":run.getErrorMessage(); SyncItemError item=new SyncItemError(); item.setRunId(run.getId()); item.setItemKey(job.getRequestPath()); item.setErrorMessage(message); item.setAttemptCount(run.getAttemptCount()); item.setStatus("DEAD_LETTER"); SyncItemError savedError=errors.save(item); DeadLetter deadLetter=new DeadLetter(); deadLetter.setItemErrorId(savedError.getId()); deadLetter.setRunId(run.getId()); deadLetter.setLastResult(message); deadLetters.save(deadLetter); run.setFailureCount(1); run.setStatus("FAILED");run.setFinishedAt(LocalDateTime.now());return runs.save(run); }
 @Transactional public DeadLetter replay(String id, String actor) { DeadLetter d=deadLetters.findById(id).orElseThrow(); if(!"PENDING".equals(d.getStatus())) throw new IllegalStateException("dead letter already replayed"); d.setReplayCount(d.getReplayCount()+1);d.setStatus("REPLAYED");d.setReplayedAt(LocalDateTime.now());d.setLastResult("manual replay requested"); DeadLetter saved=deadLetters.save(d);audit.log(actor,"INTEGRATION_DEAD_LETTER_REPLAY",id);return saved; }
