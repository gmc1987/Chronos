...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
			...
			try {
				...
				if (username != null && SecurityContextHolder.getContext().getAuthentication() == null) {
					if ("consumer".equalsIgnoreCase(type)) {
						...
						var account = this.userDetailsService.loadAccount(username);
						Integer tokenVersion = claims.get("tokenVersion", Integer.class);
						if (account == null || !java.util.Objects.equals(tokenVersion == null ? 0 : tokenVersion,
							...
							response.getWriter().write(this.objectMapper.writeValueAsString(ResultData.builder().code("401").msg("token revoked").data(null).build())); return;
						}
						if (Boolean.TRUE.equals(account.getMustChangePassword()) && !passwordChangeRequest(request)) {
				...
				response.setStatus(401);response.setContentType("application/json;charset=UTF-8");
				response.getWriter().write(this.objectMapper.writeValueAsString(ResultData.builder().code("401").msg("invalid token").data(null).build()));return;
			}
		}
	...
	private boolean passwordChangeRequest(HttpServletRequest request) {
		...
		return uri != null && uri.endsWith("/auth/account/change-password");
	}
}
