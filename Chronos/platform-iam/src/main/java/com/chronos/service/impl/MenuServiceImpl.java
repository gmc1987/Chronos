...
public class MenuServiceImpl implements IMenuService {
	...
	public void delete(String id) {
		...
		if (!menuRepository.findByParentId(id).isEmpty()) {
			throw new IllegalArgumentException("该菜单下仍有子菜单，请先删除或迁移子菜单");
		}
		var affectedRoles = roleRepository.findAll().stream()
				.filter(role -> role.getMenus() != null && role.getMenus().removeIf(item -> id.equals(item.getId())))
				.toList();
		if (!affectedRoles.isEmpty()) roleRepository.saveAllAndFlush(affectedRoles);
		roleMenuPermissionRepository.deleteByMenuId(id);
