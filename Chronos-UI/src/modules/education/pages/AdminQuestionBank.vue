  ...
  <section class="slice-page">
    ...
    <div class="crumbs">
      <el-select v-model="courseId" clearable filterable placeholder="选择课程" @change="selectCourse">
        <el-option v-for="course in courses" :key="course.id || course.courseCode" :label="course.courseName || course.name || course.courseCode" :value="course.courseCode" />
      </el-select>
    ...
    <el-dialog v-model="bankDialog" title="题库" width="560px"><el-form :model="bankForm" label-width="100px">
      <el-form-item label="课程" required><el-select v-model="bankForm.courseId" filterable style="width:100%">            <el-option v-for="c in courses" :key="c.id || c.courseCode" :label="c.courseName || c.name || c.courseCode" :value="c.courseCode" /></el-select></el-form-item>
