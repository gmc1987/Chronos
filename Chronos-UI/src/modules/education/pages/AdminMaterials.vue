...
const save=async()=>{try{await formRef.value.validate();const body={...form,materialType:form.materialType};let result=editing.value?await updateResource('materials',editing.value,body):await createResource('materials',body);const id=editing.value||result?.data?.id;if(pendingFile.value&&id){const uploaded=await uploadManagedFile(pendingFile.value,'MATERIAL',id);if(!uploaded?.data?.id)throw new Error('材料文件上传失败');await addResourceVersion('materials',id,{fileId:uploaded.data.id})}dialog.value=false;ElMessage.success('教学材料已保存');await load()}catch(e){ElMessage.error(e.message)}}
