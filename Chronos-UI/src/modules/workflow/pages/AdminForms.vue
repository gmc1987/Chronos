...
const loadFields = async () => {
  const res = await listFormFields(currentForm.value.id)
  fields.value = res?.data || []
  await loadFieldDictionaries(fields.value)
}
...
const openField = (field) => {
  ...
  const config = optionConfig(fieldDraft.value.optionsJson)
  optionSource.value = !Array.isArray(config) && config.dictCode ? 'DICTIONARY' : 'CUSTOM'
  dictionaryCodeValue.value = !Array.isArray(config) ? config.dictCode || '' : ''
  optionText.value = Array.isArray(config)
    ? config.map((item) => `${item.value}|${item.label}`).join('\n')
    : ''
  showFieldDialog.value = true
}

const saveField = async () => {
  if (optionField.value && optionSource.value === 'DICTIONARY' && !dictionaryCodeValue.value.trim()) {
    ElMessage.warning('请输入字典编码')
    return
  }

  const customOptions = optionText.value
    .split('\n')
    .map((line) => line.trim())
    .filter(Boolean)
    .map((line) => {
  ...
  const optionsConfig = !optionField.value
    ? []
    : optionSource.value === 'DICTIONARY'
      ? { source: 'DICTIONARY', dictCode: dictionaryCodeValue.value.trim() }
      : customOptions
  const payload = {
    ...fieldDraft.value,
    optionsJson: JSON.stringify(optionsConfig),
  }
  if (payload.id) await updateFormField(payload)
  else await createFormField(payload)
...
const parseOptions = (json) => {
  const config = optionConfig(json)
  return Array.isArray(config) ? config : dictionaryData[config?.dictCode] || []
}

const loadFieldDictionaries = async (items) => {
  const codes = [...new Set((items || [])
    .map((field) => optionConfig(field.optionsJson)?.dictCode)
    .filter(Boolean))]
  await Promise.all(codes.map(async (code) => {
    if (dictionaryData[code]) return
    ...
    try {
      ...
      dictionaryData[code] = []
      ElMessage.warning(`字典 ${code} 加载失败`)
    }
  }))
}
